  ...
  try {
    ...
    if (user.verification_expires) {
      ...
      if (new Date() < new Date(created.getTime() + 60 * 1000)) {
        return jsonError('Please wait at least 1 minute before requesting another reset', 429);
      }
    }

    const { code, expires, token } = generateVerificationCode(String(user.id), 'password_reset');
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr><td style="padding:32px;">
          <p style="color:#334155;font-size:15px;line-height:1.6;margin:0 0 24px;">Enter this code in the app:</p>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            <tr><td align="center">
              <a href="${resetUrl}" target="_blank" style="display:inline-block;background:linear-gradient(135deg,#1e293b,#334155);color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 40px;border-radius:12px;box-shadow:0 4px 12px rgba(30,41,59,0.3);">Reset Password</a>
            </td></tr>
          </table>
